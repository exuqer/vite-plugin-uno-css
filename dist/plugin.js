...
export function UnoCSSPlugin(options = {}) {
    const { presets = [presetUno(), presetAttributify(), presetIcons()], theme = {}, shortcuts = {}, rules = [], mode = 'auto', dev = false, } = options;
    ...
    const classMappingCache = isDevMode || isBuildMode ? new Map() : null;
    ...
    const isSupportedFile = (id, type) => {
        return SUPPORTED_EXTENSIONS[type].some(ext => id.endsWith(ext));
    };
    // Вспомогательные функции
    const processCSSAssets = async (bundle) => {
        if (!cssProcessor || !classMappingCache)
            return;
    ...
    const processJSAssets = async (bundle) => {
        if (!classMappingCache)
            return;
        for (const fileName in bundle) {
            const file = bundle[fileName];
    ...
    const processJavaScriptCode = async (code, classMappingCache) => {
    ...
    const generateUnoCSS = async () => {
        if (!classMappingCache || !uno)
            return { allUnoClasses: new Set(), finalCSS: [] };
    ...
    const updateHTMLFiles = async (bundle, cssFileName) => {
        if (!htmlProcessor || !classMappingCache)
            return;
        for (const fileName in bundle) {
            const file = bundle[fileName];
