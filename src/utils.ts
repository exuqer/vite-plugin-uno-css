 ...
 */
export function convertPropertyToUnoClass(property: string, value: string): string[] | null {
  ...
  if (property === 'width') {
    if (trimmedValue === '100') return null;
    if (trimmedValue === '100%') return ['w-full'];
    if (trimmedValue === '100vw') return ['w-screen'];
    if (/^calc\(.+\)$/.test(trimmedValue)) return [`w-[${trimmedValue}]`];
    if (/^-?\d+(\.\d+)?px$/.test(trimmedValue)) return [`w-[${trimmedValue}]`];
    return [`w-[${trimmedValue}]`];
  }
  if (property === 'height') {
    if (trimmedValue === '100') return null;
    if (trimmedValue === '100%') return ['h-full'];
    if (trimmedValue === '100vh') return ['h-screen'];
    if (/^calc\(.+\)$/.test(trimmedValue)) return [`h-[${trimmedValue}]`];
 ...
 */
function processShorthandProperty(property: string, value: string): string[] {
 ...
 */
export function propertyValueToString(value: any): string {
 ...
 */
export function extractSelectors(prelude: any): string[] {
 ...
 */
export function extractProperties(block: any): any[] {
